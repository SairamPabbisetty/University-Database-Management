...
public class FacLeaveDetails extends JFrame implements ActionListener {
	
	Choice cempid;
	Conn c;
	ResultSet rs1, rs2;
	JButton search, save, insert, update, close;
	JLabel lblempid, lblempidValue, lbldate, lbldateValue, lblduration, lbldurationValue;
	...
	public void actionPerformed(ActionEvent ae) {
		if(ae.getSource()==search) {
			try {
				rs2 = c.s.executeQuery("select * from teacherleave where empid='"+cempid.getSelectedItem()+"'");
				...
				if(rs2.next()) {
					...
					lblempidValue = new JLabel(rs2.getString("empid"));
					lblempidValue.setBounds(200, 150, 150, 30);
					lblempidValue.setFont(new Font("serif", Font.BOLD, 20));
					add(lblempidValue);
